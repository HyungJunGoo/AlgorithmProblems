...
class ATM{
public:
    int get_minimum_time(vector<int> clients_time, int number_of_clients){
        vector<pair<int, int> > pairs (number_of_clients);
        for(int i=0; i<number_of_clients; i++){
            pairs[i] = make_pair(i+1, clients_time[i]);
        }
...
int main()
{
    ...
    ATM atm;
    cout << atm.get_minimum_time(t, num) << endl;
    return 0;
}